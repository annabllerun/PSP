...
#if TM_COMPILE_SCHEDULING_SUPPORT
    /*!
     *  Why don't we simply use SCHEDULING_STRATEGY_COUNT?
     *  -> The members of SchedulingStrategy may not be strictly increasing, so there may be
     *     gaps in the interval [0,SCHEDULING_STRATEGY_COUNT] and more importantly, some
     *     strategies might have a value >=SCHEDULING_STRATEGY_COUNT.
     *     Gaps are no problem for the TM engine.
     */
    #if VERSUCH >= 5
...
#define SUBP(INDEX, CALL, PARAM, RANGE) \
    case INDEX: { \
        ...
        SUBP(1, tm_startProg, 0, MAX_NUMBER_OF_PROGRAMS)
#if TM_COMPILE_KILL_SUPPORT
        SUBP(2, tm_killProc, os_getCurrentProc(), MAX_NUMBER_OF_PROCESSES)
#endif
#if TM_COMPILE_PRIORITY_SUPPORT
        SUBP(3, tm_priority, os_getCurrentProc(), MAX_NUMBER_OF_PROCESSES)
#endif
#if TM_COMPILE_SCHEDULING_SUPPORT
        SUBP(4, tm_scheduling, os_getSchedulingStrategy(), SS_MAX_COUNT)
#endif
#if TM_COMPILE_HEAP_SUPPORT
        SUBP(5, tm_heap, 0, TM_HEAP_SUPPORT)
#endif
#undef SUBP
        default:
 ...
 *  NAME: the literal name of the page-handler to create.
 *  CHILD: the sub-page of this page.
 *  CHILDARG: the initial index to call the sub-page with.
 *  CHILDRANGE: the range of the sub-page.
 */
#define make_pagehandler(NAME, CHILD, CHILDARG, CHILDRANGE, REQ, REQFLAG, ARG) \
    ...
    static TM_PAGE(NAME) { \
        ...
        pageHandlerWrapper( \
                            p, result \
                            , NAME##_user, ((CHILD) == tm_null)?0:(CHILD) \
                            , (CHILDARG), (CHILDRANGE) \
                            , REQ, ra, OS_RAF_##REQFLAG); \
    } \
 ...
 */
static void pageHandlerWrapper(ParamStack const* p, PageResult* result,
    bool (*pageHnd)(ParamStack const*, PageState*), tm_page* childHnd,
    uint16_t childArg, uint16_t childRange, PermissionRequest pr,
    RequestArgument ra, RequestArgumentFlag raf) {
...
/*!
 *  This page allows you to select a program (!) and then
 *  execute this program (i.e. make a process of it).
 */
make_pagehandler(tm_startProg, tm_startProg_exec, 0, 1, OS_PR_START_PROG_SELECT, null, 0) {
    uint16_t const page = peekStack(0).param;
    if (!*os_getProgramSlot(page)) {
        return false;
    }
    lcd_writeProgString(PSTR("Start prog $"));
    lcd_writeDec(page);
    lcd_writeChar('/');
    lcd_writeDec(os_getNumberOfRegisteredPrograms());
    lcd_line2();
    lcd_writeProgString(PSTR(" @"));
    lcd_writeHexWord((uint16_t)(*os_getProgramSlot(page)));
    return true;
}

/*!
 *  This page has the task to only execute the program as
 *  selected by 'tm_startProg' (its parent page) and tell the user
 *  whether this was successful or not.
 *  We always return true to the TM.
 */
make_pagehandler(tm_startProg_exec, tm_null, 0, 0, OS_PR_START_PROG, prog, peekStack(1).param) {
    uint16_t const prog = peekStack(1).param;
    if (os_getNumberOfActiveProcs() < MAX_NUMBER_OF_PROCESSES) {
        lcd_writeProgString(PSTR("Starting $"));
        lcd_writeDec(prog);
        ProcessID const pid = os_exec(prog, DEFAULT_PRIORITY);
 ...
 */
static bool procMutator(ParamStack const* p, char const* note, uint32_t stateCondition) {
    uint16_t const page = peekStack(0).param;
    ...
    lcd_writeDec(os_getNumberOfActiveProcs());
    lcd_line2();
    lcd_writeProgString(PSTR("(of program $"));
    lcd_writeDec(os_getProcessSlot(page)->progID);
    lcd_writeChar(')');
    return true;
}
 ...
 *  \returns true.
 */
static bool procMutatorConfirm(ParamStack const* p, char const* note1, char const* noteIdle, bool (*action)(ProcessID)) {
    uint16_t const proc = peekStack(1).param;
    if (proc || !noteIdle) {
...
make_pagehandler(tm_priority_changeH, tm_priority_changeL, 0, 16, OS_PR_PRIORITY, null, 0) {
    uint16_t const proc = peekStack(2).param;
    // Index to start the sub-page with.
...
make_pagehandler(tm_priority_changeL, tm_priority_set, 0, 1, OS_PR_PRIORITY, null, 0) {
    uint16_t const proc = peekStack(3).param;
    priorityConstText(proc, true);
...
make_pagehandler(tm_priority_set, tm_null, 0, 0, OS_PR_PRIORITY, pid, peekStack(4).param) {
    ...
    os_getProcessSlot(peekStack(4).param)->priority
        = ((peekStack(2).param & 0xF) << 4)
          + ((peekStack(1).param & 0xF));
    tm_done();
 ...
 *  \param offsetOut Pointer to a location where to store the length of the returned string.
 ...
 */
typedef char const* StrategyNameLookup(uint16_t index, uint16_t* stringLengthOut);
    ...
    static char const* NAME(uint16_t ss, uint16_t* stringLengthOut) { \
        struct { \
            ...
            char const* name; \
                            ...
                            STRATEGIES \
        {ss, UNDEFSTRING} /* <--- sentinel element (guarantees the loop terminates!) */ \
                          }; \
        if (stringLengthOut) \
 ...
 *  \returns true.
 */
static bool strategySelector(ParamStack const* p, StrategyNameLookup* names, uint16_t curr) {
    uint16_t const select = peekStack(0).param;
    uint16_t length;
    char const* const name = names(select, &length);
    if (!name) {
        return false;
    }
 ...
 *  \returns true.
 */
static bool strategyChanger(ParamStack const* p, StrategyNameLookup* names, uint16_t curr,
                            void (*set)(void*, uint16_t), void* passThrough) {
    uint16_t const select = peekStack(1).param;
    if (select == curr) {
...
makeStrategyNameLookup(getSchedulingStratNames, 0x18, 0,
    // 123456789abcdef0123456789ABCDEF0
    {OS_SS_RUN_TO_COMPLETION,         PSTR("<Run To Completion>    ")},
    {OS_SS_RANDOM,                    PSTR("<Random>               ")},
    {OS_SS_EVEN,                      PSTR("<Even>                 ")},
    {OS_SS_ROUND_ROBIN,               PSTR("<Round Robin>          ")},
    {OS_SS_INACTIVE_AGING,            PSTR("<Inactive Aging>       ")},
    #if VERSUCH >= 5
...
static char const* getHeapName(uint8_t ram) {
    Heap* const heap = os_lookupHeap(ram);
    return heap ? (heap->name) : PSTR("unnamed");
}
...
makeStrategyNameLookup(getAllocationStratNames, 0x10, 0,
    // 123456789abcdef0123456789ABCDEF0
    {OS_MEM_FIRST, PSTR("<First Fit>    ")},
    {OS_MEM_NEXT,  PSTR("<Next Fit>     ")},
    {OS_MEM_BEST,  PSTR("<Best Fit>     ")},
    {OS_MEM_WORST, PSTR("<Worst Fit>    ")},
)
...
static MemValue derefMap(Heap const* heap, MemAddr usePtr) {
    uint16_t const uOff = usePtr - os_getUseStart(heap);
...
make_pagehandler(tm_heap_chunks, tm_null, 0, 0, OS_PR_SHOW_HEAP, null, 0) {
    Heap* const heap = os_lookupHeap(peekStack(2).param);
    uint16_t const addr = os_getUseStart(heap) + peekStack(0).param;
    MemValue const owner = derefMap(heap, addr);
    ...
    uint16_t const length = os_getChunkSize(heap, addr);
    lcd_goto(2, 9);
...
make_pagehandler(tm_heap_erase, tm_heap_erase2, 0, 1, OS_PR_ERASE_HEAP, heapId, peekStack(2).param) {
    ...
    lcd_writeProgString(getHeapName(peekStack(2).param));
    lcd_writeChar('?');
    return true;
}
